...
class BatchNormalization:
    def __init__(self,
        ...
        # # print("paramsG_"+ str(self.DataNum) + "_" + self.LayerNum +".pickle") #--追加-----------
        # self.paramsG = {"gamma":self.gamma, "beta":self.beta, "mean":self.moving_mean, "var":self.moving_var}  #--追加
        # with open("paramsG_"+ str(self.DataNum) + "_" + self.LayerNum +".pickle", "wb") as f:
        #     pickle.dump(self.paramsG, f)
    ...
    def __forward(self, x, train_flg, epsilon=1e-8):
        ...
        if train_flg:
            ...
            #--追加----------- #--追加----------- #--追加-----------
            # print("paramsG_"+ str(self.DataNum) + "_" + self.LayerNum +".pickle") #--追加-----------
            self.paramsG = {"gamma": self.gamma, "beta": self.beta,
                            "mean": self.moving_mean, "var": self.moving_var}  # --追加
            with open("paramsG_" + str(self.DataNum) + "_" + self.LayerNum + ".pickle", "wb") as f:
                pickle.dump(self.paramsG, f)
            # print(self.paramsG["gamma"])
        else:
            """
            予測時
            """
...
class MaxPooling:
    ...
    def forward(self, x):
        ...
        # 出力サイズ
        out_h = (H + 2 * self.pad -
                 self.pool_h) // self.stride + 1  # 出力の高さ(端数は切り捨てる)
        out_w = (W + 2 * self.pad -
                 self.pool_w) // self.stride + 1  # 出力の幅(端数は切り捨てる)
